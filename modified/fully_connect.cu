...
void FullyConnect::forward() {
  ...
  dim3 numBlocks(
      ceil((float)Output_width / TILE_WIDTH),
      ceil((float)Output_height / TILE_WIDTH)); // bx = O_WIDTH, by = O_HEIGH
  ...
  gemm_with_bias_h<<<numBlocks, threadsPerBlock>>>(
      X_pointer, W_pointer, Output_pointer, b_pointer, X_height, X_width,
      W_width, Output_height, Output_width);
}
...
void FullyConnect::backward() {
  ...
  dim3 numBlocks_back_dE_dW(
      ceil((float)W_width / TILE_WIDTH),
      ceil((float)W_height / TILE_WIDTH)); // bx = O_WIDTH, by = O_HEIGH

  // cout << "FC backward numBlocks_transpose_X end" << endl << fflush;
  gemm_h<<<numBlocks_back_dE_dW, threadsPerBlock>>>(
      XT_pointer, Output_pointer, Wgrad_pointer, XT_height, XT_width,
      Output_width, W_height, W_width);
  ...
  dim3 numBlocks_back_dE_dX(
      ceil((float)X_width / TILE_WIDTH),
      ceil((float)X_height / TILE_WIDTH)); // bx = O_WIDTH, by = O_HEIGH
  gemm_h<<<numBlocks_back_dE_dX, threadsPerBlock>>>(
      Output_pointer, WT_pointer, X_pointer, Output_height, Output_width,
      WT_width, X_height, X_width);
  ...
  grad_descent<<<blockDim_W, 1024>>>(W_pointer, Wgrad_pointer,
                                     W_width * W_height);
