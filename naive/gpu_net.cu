...
void GPU_Net::train(host_vector<host_vector<float>> &Xtrain,
  ...
  for (int epoch = 1; epoch <= 100; epoch++) {
    ...
    while (minibatch_index < NUM_TRAIN / MINIBATCH) {
      ...
      sm1.accuracy(MINIBATCH, 10, Xtrain, Ytrain, fc3.Output_c, minibatch_index,
                   correct_num);
      sm1.softmax(MINIBATCH, 10, Ytrain, fc3.Output_c);
      sm1.cross_entropy_loss(MINIBATCH, Ytrain, fc3.Output_c, 10, sm1.loss,
                             minibatch_index);
      sm1.softmax_backward(MINIBATCH, Ytrain, fc3.Output_c, sm1.delta_c, 10,
                           minibatch_index);
