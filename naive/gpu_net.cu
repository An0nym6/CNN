...
void GPU_Net::train(host_vector<host_vector<float>> &Xtrain,
host_vector<int> &Ytrain) {
dim3 threadsPerBlock(TILE_WIDTH, TILE_WIDTH);
cout << "GPU_Net Train Start" << endl << fflush;
int minibatch_index;
double elapsed;
...
sm1.accuracy(MINIBATCH, 10, Xtrain, Ytrain, fc4.Output_c, minibatch_index, correct_num);

sm1.softmax(MINIBATCH, 10, Ytrain, fc4.Output_c);

sm1.cross_entropy_loss(MINIBATCH, Ytrain, fc4.Output_c, 10, sm1.loss, minibatch_index);

//// SoftMax delta
sm1.softmax_backward( MINIBATCH, Ytrain, fc4.Output_c, sm1.delta_c, 10, minibatch_index);
...
dim3 threadsPerBlock(TILE_WIDTH, TILE_WIDTH);
cout << "GPU_Net Test Start" << endl << fflush;
...
sm1.accuracy(MINIBATCH, 10, Xtest, Ytest, fc3.Output_c, minibatch_index,
correct_num);
sm1.softmax(MINIBATCH, 10, Ytest, fc3.Output_c);
sm1.cross_entropy_loss(MINIBATCH, Ytest, fc3.Output_c, 10, sm1.loss,
minibatch_index);
sm1.softmax_backward(MINIBATCH, Ytest, fc3.Output_c, sm1.delta_c, 10,
minibatch_index);
clock_gettime(CLOCK_REALTIME, &finish);
elapsed += ((double)finish.tv_sec - start.tv_sec) +
((double)finish.tv_nsec - start.tv_nsec) / 1000000000.0;
